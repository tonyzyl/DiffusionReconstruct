...
from utils.attn_utils import FieldsVisionConfig, FieldsVisionModelWithProjection
...
class diffuserUNet2DCondition(
    ModelMixin, ConfigMixin, FromOriginalModelMixin, UNet2DConditionLoadersMixin, PeftAdapterMixin
):
    # modified from UNet2DConditionModel
    ...
    def __init__(
        ...
        field_encoder_dict: Optional[dict] = None,
    ):
        super().__init__()
        ...
        if field_encoder_dict is not None:
            self.field_encoder = FieldsVisionModelWithProjection(FieldsVisionConfig(**field_encoder_dict))
        else:
    ...
    def forward(
        ...
        if self.field_encoder is not None:
            #encoder_hidden_states = self.field_encoder(conditioning_tensors).last_hidden_state
            if self.field_encoder.config.output_hidden_state:
                encoder_hidden_states = self.field_encoder(conditioning_tensors).last_hidden_state
            else:
                encoder_hidden_states = self.field_encoder(conditioning_tensors).image_embeds
                encoder_hidden_states = encoder_hidden_states.unsqueeze(1) # (B, proj_dim) -> (B, 1, proj_dim)
        else:
            if encoder_hidden_states is None:
                raise ValueError(
                    "encoder_hidden_states should be provided when `field_encoder` is not defined in the model."
                )
...
class diffuserUNet2DCFG(diffuserUNet2DCondition):
    @register_to_config
    def __init__(
        self,
        sample_size: Optional[int] = None,
        ...
        super().__init__(sample_size=sample_size, in_channels=in_channels, out_channels=out_channels,
                         ...
                         time_cond_proj_dim=time_cond_proj_dim, conv_in_kernel=conv_in_kernel, conv_out_kernel=conv_out_kernel,
                         projection_class_embeddings_input_dim=projection_class_embeddings_input_dim, 
                         ...
                         field_encoder_dict=field_encoder_dict)

        assert self.field_encoder is not None, "When using CFG, we need the field encoder"
        assert self.field_encoder.config.output_hidden_state == False, "When using CFG, we need the image_embeds to do the conditioning"

        # get the time_embed_dim
        time_embed_dim = self.time_embedding.linear_2.out_features
        if self.field_encoder.config.projection_dim != time_embed_dim:
            self.image_embed_proj = nn.Linear(self.field_encoder.config.projection_dim, time_embed_dim)
        else:
            self.image_embed_proj = nn.Identity(time_embed_dim, time_embed_dim)
    ...
    def forward(
        ...
        """
        Add the output of the field encoder to the temb
        """
        ...
        image_embeds = self.field_encoder(conditioning_tensors).image_embeds
