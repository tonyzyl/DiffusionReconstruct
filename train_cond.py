...
from pipelines.pipeline_inv_prob import InverseProblem2DCondPipeline
from losses.metric import metric_func_2D

logger = get_logger(__name__, log_level="INFO")

def log_validation(phase_name, config, epoch, unet, noise_scheduler_config, accelerator, known_latents=None, orig_batch=None, mask=None):
    logger.info(f"Running evaluation on {phase_name} set")
    noise_scheduler = instantiate_from_config(noise_scheduler_config)
    pipeline = InverseProblem2DCondPipeline(unet, scheduler=noise_scheduler)
    ...
    sample_images = pipeline(
        ...
        generator=generator,
        mask=mask[:config.eval_batch_size],
    ...
    for i in range(sample_images.shape[1]):
        ...
        ground_truth = convert_to_rgb(orig_batch[:config.eval_batch_size, i].reshape(-1, 1, image_dim, image_dim))
        images_list.append(make_grid(torch.stack(tmp_image)))
        GT_list.append(make_grid(torch.stack(ground_truth)))

    err_RMSE, err_nRMSE, err_CSV = metric_func_2D(sample_images, orig_batch[:config.eval_batch_size], 
                                                  mask=mask[:config.eval_batch_size])
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="xattn",
        help="The type of conditioning to use. Options: xattn, cfg.",
    )
    parser.add_argument(
...
def main(args):
    ...
    #assert len(general_config.known_channels) == unet_config["field_encoder_dict"]["num_channels"], \
    #        "The number of known channels must match the number of field encoder channels."

    set_seed(general_config.seed)

    if args.conditioning_type == "xattn":
        unet_mdl_cls = diffuserUNet2DCondition
    elif args.conditioning_type == "cfg":
        ...
        def load_model_hook(models, input_dir):
            if ema_config.use_ema:
                ...
                load_model = EMAModel.from_pretrained(
                    os.path.join(input_dir, "unet_ema"), unet_mdl_cls #, foreach=ema_config.foreach # not yet released in v0.29.2
                )
            ...
            for _ in range(len(models)):
                ...
                load_model = unet_mdl_cls.from_pretrained(input_dir, subfolder="unet")
    ...
    with accelerator.main_process_first():
        ...
        logger.info(f"Loading data from {dataloader_config.data_dir}")
        # TODO: load in memory cause trouble on server
        train_dataloader, val_dataloader, _ = pdedata2dataloader(**dataloader_config, generator=generator, load_in_memory=False)
    ...
    for epoch in range(first_epoch, general_config.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if general_config.cond_drop_prob is not None:
                    no_cond_idx = torch.rand(clean_images.shape[0], device=clean_images.device) < general_config.cond_drop_prob
                    mask[no_cond_idx] = 0
                    interpolated_fields[no_cond_idx] = 0
                noise = noise * (1 - mask)
                ...
                conditioning_tensors = torch.cat((interpolated_fields, mask[:, [general_config.known_channels[0]]]), dim=1) # Added a mask channel to interpolated_fields
                ...
                if general_config.do_edm_style_training:
                    ...
                    if "EDM" in noise_scheduler_config.target:
                        model_pred = noise_scheduler.precondition_outputs(noisy_images, model_pred, sigmas) # the last (or more) channel is the mask
                ...
                if general_config.snr_gamma is None:
                    if weighting is not None:
                        #loss = torch.sum(weighting.float() * ((model_pred.float() - target.float())**2) * unknown_mask, dim=[-2, -1], keepdim=True) / num_unknowns
                        loss = torch.mean(
                        ...
                        #loss = torch.sum(((model_pred.float() - target.float())**2) * unknown_mask, dim=[-2, -1], keepdim=True) / num_unknowns
                        loss = torch.mean(
                    ...
                    #TODO: add scale by unknown_mask
                    loss = F.mse_loss(model_pred.float(), target.float(), reduction="none")
                    loss = loss.mean(dim=list(range(1, len(loss.shape)))) * mse_loss_weights
                    loss = loss.mean()

                # Gather the losses across all processes for logging (if we use distributed training).
                avg_loss = accelerator.gather(loss.repeat(dataloader_config.batch_size)).mean()
        ...
        if accelerator.is_main_process:
            ...
            if (epoch + 1) % general_config.save_image_epochs == 0 or epoch == general_config.num_epochs - 1:
                ...
                tmp_mask = create_scatter_mask(tmp_batch, channels=general_config.known_channels, ratio=0.015625, device=accelerator.device)
                tmp_interpolated_fields = vt(tmp_batch, mask=tmp_mask)
                log_validation('val', general_config, epoch, unwrap_model(unet), noise_scheduler_config, accelerator=accelerator,
                                known_latents=tmp_interpolated_fields, orig_batch=tmp_batch, mask=tmp_mask)
                if ema_config.use_ema:
                    # Restore the UNet parameters.
                    ema_model.restore(unet.parameters())


            # Training will get stuck if call in accelerator.wait_for_everyone() & is_main_process
